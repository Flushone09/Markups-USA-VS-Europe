...
/*Lancer la macro */
%import ;
/* fixer l'erreur de belcap_qi et FR */
 %macro drop_val_bel;
data belcap_qi; set belcap_qi; %do p= 1970 %to 2015;
keep desc code CAP_QI&p;
%end;
run; 
%mend; 
%drop_val_bel;
%macro fra(tab) ; 
data &tab; set &tab; drop AW;
...
%fra (fracap_qi); 
%fra (fracomp);
data belgo; set belgo; if _N_>42 then delete; run;
...
Run;
data in.Area_corriger ; set Area ; run;
